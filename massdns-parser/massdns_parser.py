...
class BinaryDnsResultParser:
    def __init__(self, filename, throw_incomplete=False):
        ...
        (time_size, sockaddr_storage_size, family_offset, family_size, port_size,) = struct.unpack(
            endianness + 5 * size_modifier, f.read(5 * size_len)
        )
...
if __name__ == "__main__":
    ...
    with BinaryDnsResultParser(sys.argv[1], True) as parser:
        for result in parser.results():
            ...
            print(f"{result.resolver[0]}:{result.resolver[1]}, {formatted_time}")
